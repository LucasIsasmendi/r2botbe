...
function saveFiles (filesToAddArray, cb) {
...
function saveFilesV2 (files, cb) {
  const node = new IPFS(path.join(os.tmpDir() + '/' + new Date().toString()))
  const promises = files.map(async((filePath) => {
    // const fileName = path.basename(filePath)
    try {
      const stream = awaitt(fs.createReadStream(filePath.path))
      const file = {
  ...
  const filesready = awaitt(Promise.all(promises))
  console.log('filesready: ', filesready)
  const result = awaitt(node.files.add(filesready))
  console.log('result: ', result)
  cb(filesready, result)
}

function getFiles (repoPath, multiFilesToGetByHash, cb) {
  const node = new IPFS(repoPath)
...
function getFile (repoPath, hash, cb) {
  const node = new IPFS(repoPath)
